...
def real_loss(D_out,epoch=0):
    '''Calculates how close discriminator outputs are to being real.
       param, D_out: discriminator logits
       while epoch=0, use hard labels
       return: real loss'''
...
def fake_loss(D_out,epoch=0):
    '''Calculates how close discriminator outputs are to being fake.
       param, D_out: discriminator logits
       return: fake loss'''
    batch_size = D_out.size(0)
    if epoch > 300:
        labels = torch.zeros(batch_size) + 0.05
    else:
...
def train(D, G, n_epochs, print_every=50):
    ...
    for epoch in range(first_epoch, n_epochs):
        ...
        for batch_i, real_seeds in enumerate(train_loader):
            ...
            random_noise = np.random.uniform(0, 0.0039, size=(batch_size, 1, maxsize))
            ...
            dreal = D(real_seeds)
            dreal_loss = real_loss(dreal, epoch)
            ...
            # loss of fake seeds           
            dfake = D(fake_seeds)
            dfake_loss = fake_loss(dfake, epoch)
            ...
            D_fake = D(fake_seeds)
            g_loss = real_loss(D_fake, epoch) # use real loss to flip labels
