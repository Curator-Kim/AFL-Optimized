...
class SeedDataset(data.Dataset):
    def __init__(self, seed_dir, transform=None):
        ...
        for name in self.seed_names:
            tmp_size = os.path.getsize(seed_dir+'/'+name)
            if tmp_size > self.maxsize:
                self.maxsize = tmp_size
    ...
    def padding(self, l):
        for _ in range(self.maxsize-len(l)):
            l.append(0)
...
def deconv(in_channels, out_channels, kernel_size, stride=2, padding=1, batch_norm=True):
    ...
    layers = []
    convt_layer = nn.ConvTranspose1d(in_channels, out_channels, 
                           kernel_size, stride, padding, bias=False)
...
class Generator(nn.Module):
    ...
    def __init__(self, z_size, conv_dim):
        ...
        #16x16
        self.dcv4 = deconv(self.conv_dim, 1, 4, batch_norm=False)
        #32 x 32
